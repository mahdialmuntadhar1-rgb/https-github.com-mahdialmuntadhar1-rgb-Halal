...
export default function TrustPrivacyScreen({ locale, onBackToOverview }: TrustPrivacyScreenProps) {
  ...
  const t = {
    ...
    badge: isAr ? 'تصميم يحفظ الخصوصية' : isku ? 'بۆ پاراستنی نهێنی دروستکراوە' : 'Designed for Privacy',
  };

  const principles = [
    {
      icon: <Users className="w-6 h-6 text-accent-coral" />,
      title: isAr ? 'مجتمع يركز على الزواج فقط' : isku ? 'کۆمەڵگەی تەنها کۆنکریت لەسەر هاوسەرگیری' : 'Marriage-Focused Community',
      desc: isAr
      ...
      status: isAr ? 'مدمج في التدفق التعريفي' : isku ? 'لەنێو پڕۆسەی ناساندنەکەیە' : 'Privacy controls built into the product flow'
    },
    {
      icon: <Lock className="w-6 h-6 text-[#40798C]" />,
      title: isAr ? 'ملفات تعريفية بخصوصية مصانة' : isku ? 'پڕۆفایلی تەواو نهێنی' : 'Private Profiles',
      desc: isAr
      ...
      status: isAr ? 'مدمج في تدفق العرض' : isku ? 'لە ژێر دەسەڵاتی بەکارهێنەرە' : 'Privacy controls built into the product flow'
    },
    {
      icon: <MessageCircle className="w-6 h-6 text-purple-600" />,
      title: isAr ? 'شروط التوافق الثنائي لبدء المحادثة' : isku ? 'ڕەزامەندی دوولایەنە بۆ گفتوگۆ' : 'Mutual Match Required for Chat',
      desc: isAr
      ...
      status: isAr ? 'مدمج في تدفق المحادثات' : isku ? 'لەنێو ژووری گفتوگۆی ئەپەکەدایە' : 'Privacy controls built into the product flow'
    },
    {
      icon: <Camera className="w-6 h-6 text-pink-600" />,
      title: isAr ? 'تحكم مرن بالصورة الشخصية' : isku ? 'کۆنترۆڵی پاراستنی وێنە' : 'Photo Control Settings',
      desc: isAr
      ...
      status: isAr ? 'تحكم تفاعلي مدمج' : isku ? 'کۆنترۆڵی دەستی متبادل' : 'Privacy controls built into the product flow'
    }
  ];
  ...
  const futurePlans = [
    {
      ...
      title: isAr ? 'نظام التحقق المدني والهوياتي القانوني' : isku ? 'سیستەمی نوێی ناساندنی یاسایی' : 'Civil Identity Verification Framework',
      desc: isAr
      ...
      status: isAr ? 'مخطط للتطوير مع تفعيل قاعدة البيانات' : isku ? 'پلانی بۆ دانراوە لەگەڵ سێرڤەر' : 'Backend verification planned'
    },
    {
      icon: <Flag className="w-6 h-6 text-amber-600" />,
      title: isAr ? 'أدوات التبليغ التلقائي والحظر الكامل' : isku ? 'ئامرازەکانی بلۆککردن و ڕاپۆرتکردن' : 'Automated Block & Report Infrastructure',
      desc: isAr
      ...
      status: isAr ? 'مخطط للتطوير النهائي' : isku ? 'پلانی بۆ دانراوە بۆ بەردەستبوونی سێرڤەر' : 'Report and block tools planned'
    },
    {
      icon: <Shield className="w-6 h-6 text-[#40798C]" />,
      title: isAr ? 'بروتوكولات حماية وتشفير قواعد البيانات' : isku ? 'پاراستنی پێشکەوتووی داتا لەسەر سێرڤەر' : 'Durable Data Protection Policies',
      desc: isAr
      ...
      status: isAr ? 'مستعد للتطبيق مع قواعد البيانات' : isku ? 'تەواوکاری جێگیر لەگەڵ جێبەجێبوونی سێرڤەر' : 'Data protection will be implemented with backend'
    }
  ];
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10" id="trust-privacy-center-v3">
      ...
      <div className="flex justify-between items-center pb-4 border-b border-stone-200/50">
        ...
        >
          ✕ {isAr ? 'إغلاق والعودة للرئيسية' : isku ? 'داخستن و گەڕانەوە' : 'Close and Back'}
        </button>
      ...
      <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/20 max-w-3xl mx-auto text-left rtl:text-right flex items-start gap-3">
        ...
        <div className="space-y-1.5">
          <p className="text-xs font-bold text-amber-800">
            {isAr ? 'تنويه الأمان الشفاف' : isku ? 'تێبینی چاودێری متمانەی ڕوون' : 'App Transparency & Disclaimer Notice'}
          </p>
      ...
      <div className="space-y-6 pt-2">
        <div className="text-left rtl:text-right border-l-4 border-accent-coral pl-3 rtl:border-l-0 rtl:border-r-4 rtl:pr-3">
          <h3 className="font-serif font-black text-warm-charcoal text-lg">
            {isAr ? '١. أدوات حماية الخصوصية المفعلة في واجهة التطبيق' : isku ? '١. ڕێبەرە متبادلە چالاکراوەکان لەناو ئەپەکەدا' : '1. Privacy Controls Built into the Product Flow'}
          </h3>
          <p className="text-[#6B635B] text-xs font-semibold mt-0.5">
            {isAr ? 'الآليات المدمجة حالياً ضمن تدفق الاستخدام للتجربة والتحقق:' : isku ? 'ئەو تایبەتمەندییانەی ئێستا بە شێوەی کارا دروستکراون بۆ ناساندن:' : 'Security behaviors active within the client experience:'}
          </p>
        </div>
      ...
      <div className="space-y-6 pt-4">
        <div className="text-left rtl:text-right border-l-4 border-emerald-600 pl-3 rtl:border-l-0 rtl:border-r-4 rtl:pr-3">
          <h3 className="font-serif font-black text-warm-charcoal text-lg">
            {isAr ? '٢. بروتوكولات الأمان المخطط لتنفيذها مع خادم البيانات' : isku ? '٢. سیستەمە ئاسایشییە پلاندارێژراوەکان لەگەڵ بەستنەوەی سێرڤەر' : '2. Advanced Protections Planned for Backend Integration'}
          </h3>
          <p className="text-[#6B635B] text-xs font-semibold mt-0.5">
            {isAr ? 'الخدمات التي ستدخل حيز التنفيذ بمجرد الاتصال بخوادم التخزين الحية للإنتاج:' : isku ? 'ئەو کارانەی چالاک دەبن کاتێک سێرڤەری گشتی بەسترایەوە:' : 'Durable safety layers activated upon deployment of cloud database schemas:'}
          </p>
        </div>
      ...
      <div className="p-6 sm:p-8 rounded-[2rem] bg-[#40798C]/5 border border-[#40798C]/15 text-center space-y-4 max-w-4xl mx-auto">
        <p className="text-xs font-mono font-bold tracking-widest text-[#40798C] uppercase">
          {isAr ? 'الاحترام والوقار • ميثاق الشرف' : isku ? 'ڕێزگرتن و بەڵێنامە • متمانە' : 'DIGNITY, VALUES & MARRIAGE ASSURANCE'}
        </p>
        <p className="text-xs text-[#6B635B] max-w-2xl mx-auto leading-relaxed font-semibold">
          ...
          {isku && "کۆمەڵگەکەمان لەسەر بەهای ڕاستگۆیی ڕاوەستاوە. تکایە بە نیازی جدییەوە بەشداری بکەن و ڕێز لە کۆنترۆڵە ڕەوشتییەکان بگرن بۆ بنیادنانی خێزانێکی تەندروست."}
        </p>
      </div>
