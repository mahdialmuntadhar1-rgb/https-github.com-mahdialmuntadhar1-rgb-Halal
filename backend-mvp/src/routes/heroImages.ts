...
export async function handleHeroImages(ctx: RequestContext): Promise<Response | null> {
  ...
  if (path === '/hero-images' && request.method === 'GET') {
    const rows = await env.DB.prepare(
      'SELECT * FROM halal_hero_images WHERE active = 1 ORDER BY sort_order ASC, created_at DESC',
    ).all();
    return json({ heroImages: rows.results || [] });
  }

  if (path === '/admin/hero-images' && request.method === 'POST') {
    requireAdmin(ctx);
    ...
    const nextOrder = await env.DB.prepare('SELECT COALESCE(MAX(sort_order), 0) + 1 AS next_order FROM halal_hero_images')
      .first<{ next_order: number }>();
    const id = uuid();
    await env.DB.prepare(
      'INSERT INTO halal_hero_images (id, title, image_url, alt_text, active, sort_order) VALUES (?, ?, ?, ?, ?, ?)',
    )
      .bind(id, title, imageUrl, altText, 1, Number(nextOrder?.next_order || 1))
      .run();
    const heroImage = await env.DB.prepare('SELECT * FROM halal_hero_images WHERE id = ?').bind(id).first();
    return json({ heroImage }, 201);
  }

  const adminMatch = path.match(/^\/admin\/hero-images\/([^/]+)$/);
  if (adminMatch && request.method === 'PUT') {
    requireAdmin(ctx);
    const id = decodeURIComponent(adminMatch[1]);
    const body = await readJson(request);
    const existing = await env.DB.prepare('SELECT * FROM halal_hero_images WHERE id = ?').bind(id).first<Record<string, unknown>>();
  ...
  if (adminMatch && request.method === 'DELETE') {
    requireAdmin(ctx);
    await env.DB.prepare('DELETE FROM halal_hero_images WHERE id = ?').bind(decodeURIComponent(adminMatch[1])).run();
    return noContent();
  }
