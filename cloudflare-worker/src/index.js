...
export default {
  async fetch(request) {
    ...
    // PUT /requests/:id/decline - Decline a match request
    if (path.startsWith("/requests/") && path.endsWith("/decline") && request.method === "PUT") {
      const matchId = path.split("/")[2];
      let declined = null;

      matches = matches.map(m => {
        if (m.id === matchId) {
          declined = { ...m, requestStatus: "declined" };
          return declined;
        }
        return m;
      });

      if (!declined) return json({ message: "Match not found" }, 404);
      return json({ success: true, match: declined });
    }
    ...
    // POST /community/posts/:id/vote - Vote on a post (alias for like)
    if (path.match(/^\/community\/posts\/[^/]+\/vote$/) && request.method === "POST") {
      const postId = path.split("/")[3];
      let post = communityPosts.find(p => p.id === postId);
      if (!post) return json({ message: "Post not found" }, 404);
    ...
    // POST /community/posts/:id/status - Update post status (admin)
    if (path.match(/^\/community\/posts\/[^/]+\/status$/) && request.method === "POST") {
      const postId = path.split("/")[3];
      const body = await readJson(request);
      let post = communityPosts.find(p => p.id === postId);
      if (!post) return json({ message: "Post not found" }, 404);
      post.status = body.status || "active";
      return json({ success: true, post });
    }

    // POST /community/posts/:id/feature - Feature/unfeature a post (admin)
    if (path.match(/^\/community\/posts\/[^/]+\/feature$/) && request.method === "POST") {
      const postId = path.split("/")[3];
      let post = communityPosts.find(p => p.id === postId);
      if (!post) return json({ message: "Post not found" }, 404);
