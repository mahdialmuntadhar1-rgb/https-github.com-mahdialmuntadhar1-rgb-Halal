...
export default function AccountPlaceholderScreen({
  ...
  const handleDemoAuthClick = (e: React.FormEvent) => {
    ...
    triggerToast(
      locale === 'en' 
        ? `?? Sandbox Demo: ${action} via ${method} simulated. Backend database connection is required for live accounts.` 
        : `?? ?????? ???????: ?? ????? ??? ${action === 'Login' ? '????? ??????' : '????? ??????'} ??? ${authMethod === 'phone' ? '??????' : '??????'}. ??????? ?????? ???????? ????? ???????.`
    );
  };
  ...
  const handleUpgrade = () => {
    setTier('premium');
    triggerToast(locale === 'en' ? '? Upgraded to Zawaj Al Araqi Premium successfully!' : '? ?? ??????? ??? ?????? ??????? ?????!');
  };

  const pledges = [
    {
      title: locale === 'en' ? 'Serious Intention Only' : '??? ?????? ?????? ???',
      desc: locale === 'en' ? 'Commitment to pure, marriage-focused communications only.' : '???????? ????? ??????? ?????? ??????? ??????? ??????? ???.',
    },
    {
      title: locale === 'en' ? 'Truthful Profile Dossier' : '????? ?? ???????? ???????',
      desc: locale === 'en' ? 'Assure all age, background, and marital state details are completely honest.' : '?????? ???? ???? ???????? ???????? ??? ?????? ?????? ??????????? ??????.',
    },
    {
      title: locale === 'en' ? 'Dignified & Safe Conduct' : '???????? ??????? ?? ???????',
      desc: locale === 'en' ? 'Respectful conduct with zero offensive or casual hookup behavior.' : '????? ?????? ?????? ?? ?????? ?????? ????? ?? ?????? ????? ?? ??????.',
    }
  ];
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-black text-warm-charcoal font-serif tracking-tight flex items-center gap-2">
            ...
            <span>{locale === 'en' ? 'Account & Security Center' : '????? ?????? ???????? ??????'}</span>
          </h2>
        ...
        <div className="bg-amber-50 border border-amber-200 text-amber-800 text-[10px] sm:text-xs font-mono font-bold px-4 py-2 rounded-2xl flex items-center gap-2 shadow-sm shrink-0">
          ...
          <span>{locale === 'en' ? 'NOTICE: Backend connection required for live auth' : '??????: ??????? ????? ????? ???????? ????? ???????? ?????'}</span>
        </div>
      </div>
          ...
          <div className="bg-white/40 backdrop-blur-xl border border-white/55 p-6 sm:p-8 rounded-[2rem] shadow-xl space-y-6">
            <div className="flex justify-between items-center pb-2 border-b border-stone-200/50">
              <h3 className="font-serif font-black text-warm-charcoal text-base flex items-center gap-2">
                ...
                <span>{locale === 'en' ? 'Secure Authentication Sandbox' : '????? ????? ?????? ?????????'}</span>
              </h3>
            ...
            <div className="grid grid-cols-2 gap-1.5 bg-stone-100 p-1 rounded-xl">
              ...
              >
                ?? {locale === 'en' ? 'Sign In / Account Access' : '????? ??????'}
              </button>
              <button
              ...
              >
                ?? {locale === 'en' ? 'Register Courtship Identity' : '????? ???? ????'}
              </button>
            </div>
            ...
            <form onSubmit={handleDemoAuthClick} className="space-y-4">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-1.5 text-xs font-bold text-warm-charcoal cursor-pointer">
                  ...
                  />
                  <span>?? {locale === 'en' ? 'Iraqi Mobile Number' : '??? ?????? ???????'}</span>
                </label>
                <label className="flex items-center gap-1.5 text-xs font-bold text-warm-charcoal cursor-pointer">
                  <input
                    type="radio"
                    ...
                    className="accent-accent-coral"
                  />
                  <span>?? {locale === 'en' ? 'Secured Email Address' : '?????? ?????????? ?????'}</span>
                </label>
              </div>
              ...
              {authMethod === 'phone' ? (
                <div className="space-y-1 text-left">
                  <label className="block text-[10px] font-mono font-bold text-[#6B635B] uppercase tracking-wide">
                    {locale === 'en' ? 'Iraqi Mobile (+964 Zain/Asiacell)' : '??? ?????? (???? / ??? / ???? ???)'}
                  </label>
                  ...
                  <p className="text-[10px] text-[#A2978C] font-semibold">
                    {locale === 'en' ? 'We will simulate sending a 6-digit verification code.' : '????? ??????? ????? ??? ????? ???? ???? ?? 6 ?????.'}
                  </p>
                </div>
              ) : (
                <div className="space-y-1 text-left">
                  <label className="block text-[10px] font-mono font-bold text-[#6B635B] uppercase tracking-wide">
                    {locale === 'en' ? 'Secure Email Address' : '?????? ?????????? ??????'}
                  </label>
                  <input
                    type="email"
                    value={emailAddress}
              ...
              >
                <span>{authTab === 'login' ? (locale === 'en' ? 'Login Secure Sandbox' : '????? ???? ?????? ?????') : (locale === 'en' ? 'Create Demo Identity' : '????? ?????? ?????????')}</span>
              </button>
            </form>
          </div>
          ...
          <div className="bg-white/40 backdrop-blur-xl border border-white/55 p-6 sm:p-8 rounded-[2rem] shadow-xl space-y-6">
            <div className="space-y-1">
              <h3 className="font-serif font-black text-warm-charcoal text-base flex items-center gap-2">
                ...
                <span>{locale === 'en' ? 'ID & Civic Registry Verification hub' : '?????? ?????? ?????? ?????????'}</span>
              </h3>
            ...
            <div className="space-y-4 pt-1">
                ...
                <div className="flex items-start gap-3 text-left">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-warm-charcoal">
                      {locale === 'en' ? 'Ministry of Interior ID Validation' : '????? ??????? ??????? ???????'}
                    </h4>
              ...
              <div className="p-4 rounded-2xl bg-white/60 border border-white/40 flex justify-between items-start sm:items-center flex-col sm:flex-row gap-3">
                <div className="flex items-start gap-3 text-left">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-warm-charcoal">
                      {locale === 'en' ? 'Linked SIM Card (+964 SMS Node)' : '????? ????? ??????? ???????? (+964)'}
                    </h4>
                    <p className="text-[10px] text-[#6B635B] leading-relaxed font-semibold mt-0.5">
                      {locale === 'en' 
              ...
              <div className="p-4 rounded-2xl bg-white/60 border border-white/40 flex justify-between items-start sm:items-center flex-col sm:flex-row gap-3">
                <div className="flex items-start gap-3 text-left">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-warm-charcoal">
                      {locale === 'en' ? 'Serious Profile Manual Review' : '???????? ??????? ???????? ???????'}
                    </h4>
                    <p className="text-[10px] text-[#6B635B] leading-relaxed font-semibold mt-0.5">
                      {locale === 'en' 
                ...
                {idSubmitted ? (
                  ...
                  >
                    {locale === 'en' ? 'Submit Identity Details' : '??? ????? ??????'}
                  </button>
                )}
              </div>
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <div className="bg-white/40 backdrop-blur-xl border border-white/55 p-6 sm:p-8 rounded-[2rem] shadow-xl space-y-4">
            <h3 className="font-serif font-black text-warm-charcoal text-base flex items-center gap-2">
              ...
              <span>{locale === 'en' ? 'Courtship Premium Subscriptions' : '?????????? ?????? ???????'}</span>
            </h3>
          ...
          <div className="bg-white/40 backdrop-blur-xl border border-white/55 p-6 sm:p-8 rounded-[2rem] shadow-xl space-y-4">
            <h4 className="font-mono text-[10px] font-bold uppercase tracking-widest text-[#40798C] flex items-center gap-2 pb-2 border-b border-stone-200/50">
              ...
              <span>{locale === 'en' ? 'Ethical Pledges & Guardrails' : '????? ????? ???????? ???????? ??????'}</span>
            </h4>
