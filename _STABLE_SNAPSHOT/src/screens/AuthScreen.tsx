...
export default function AuthScreen({ locale, onAuthSuccess, triggerToast }: AuthScreenProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await api.register(name, governorate, district.trim(), email, phone.trim() || undefined, password);
      ...
      try {
        await api.updateCurrentUserProfile({ city: district.trim() });
      } catch (e) {
