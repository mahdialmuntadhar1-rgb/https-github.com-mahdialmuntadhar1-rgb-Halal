...
export default function AdminPanel({
  ...
  const handleAddSlide = async (e: React.FormEvent) => {
    ...
    try {
      await api.addHeroImage(newUrl.trim(), newTitle.trim() || 'New Slideshow Image', newIsActive);
      setNewUrl('');
      setNewTitle('');
      setNewIsActive(true);
  ...
  const handleToggleSlideActive = async (id: string, currentStatus: boolean) => {
    try {
      await api.updateHeroImage(id, { isActive: !currentStatus });
  ...
  const handleMoveSlide = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      await api.reorderHeroImages(reordered);
  ...
  const handleDismissPost = async (postId: string) => {
    try {
      ...
      await api.updateHeroImage(postId, {}); // simulated ignore
  ...
  const seedSamplePhotos = async () => {
    try {
      await api.addHeroImage('https://images.unsplash.com/photo-1465495976277-4387d4b0b4c6?auto=format&fit=crop&q=80&w=1200', 'Elegant Wedding Pathway', true);
      await api.addHeroImage('https://images.unsplash.com/photo-1515934751635-c81c6bc9a2d8?auto=format&fit=crop&q=80&w=1200', 'Traditional Courtyard Ceremony', true);
