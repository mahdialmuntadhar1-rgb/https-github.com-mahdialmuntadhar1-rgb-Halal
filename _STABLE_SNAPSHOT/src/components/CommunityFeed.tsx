...
export default function CommunityFeed({
  ...
  const fetchPosts = async () => {
    ...
    setError(null);
    try {
      const list = await api.getCommunityPosts();
      setPosts(list);
    } catch (err: any) {
  ...
  const handleCreatePost = async (e: React.FormEvent) => {
    ...
    try {
      await api.createCommunityPost(newTitle.trim(), newContent.trim(), newCategory);
      setNewTitle('');
      setNewContent('');
  ...
  const handleLikePost = async (postId: string) => {
    ...
    try {
      await api.likePost(postId, userDisplayName);
      await fetchPosts();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleAddComment = async (postId: string) => {
    ...
    try {
      ...
      await api.addComment(postId, commentText.trim(), userDisplayName, userGender);
  ...
  const handleReportComment = async (postId: string, commentId: string) => {
    try {
      await api.reportComment(postId, commentId);
